...
module.exports = function(RED) {
        ...
        node.on('input', function (msg) {
            if (msg.hasOwnProperty("payload")) {
                ...
                //if (!isNaN(node.speed)){
                    switch(node.action){
                        case 'sport':
                            sport(node.walkspeed,node.rotatespeed);
                            ...
                            break;
                        case 'eye':
                            eyeAngle(1,node.eyeangle);
                            msg.action = node.action;
                            msg.eyeangle = node.eyeangle;
                            node.status({fill: 'green',shape: 'dot',text: ''});
                            break;
                        case 'light':
                            light(node.lightLR,node.lightLG,node.lightLB,node.lightRR,node.lightRG,node.lightRB);
                            msg.action = node.action;
    ...
    function eyeAngle(power,rotate_speed){
        var buf_data = [3,power,rotate_speed];
        execAction(buf_data);
    }

    function light(lR,lG,lB,rR,rG,rB){
        var buf_data = [2,lR,lG,lB,rR,rG,rB];
        execAction(buf_data);
    }

    function sport(walk_speed,rotate_speed){
    ...
    // Node functions.
    RED.nodes.registerType("xpd-action",XpdActionNode);
